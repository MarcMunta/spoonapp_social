...
def get_friends(user):
    ...
    sent_ids = FriendRequest.objects.filter(from_user=user, accepted=True).values_list(
        "to_user", flat=True
    )
    received_ids = FriendRequest.objects.filter(
...
def get_random_users(user, limit=None):
    ...
    if city:
        ...
        if limit is not None:
            ...
            if remaining > 0:
                others = qs.exclude(id__in=[u.id for u in nearby]).order_by("?")[
                    :remaining
                ]
    ...
    if hasattr(request, "user") and request.user.is_authenticated:
        ...
        for user in all_users:
            ...
            received = FriendRequest.objects.filter(
                from_user=user, to_user=request.user, accepted=False
            ).exists()
            ...
            friend_status_list.append(
                {
                    ...
                    "status": (
                        ...
                        if friends_check
                            ...
                            "request_sent"
                            if sent
                            else "request_received" if received else "none"
                        )
                    ),
                }
            )

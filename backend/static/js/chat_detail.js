    document.addEventListener('DOMContentLoaded', function () {
        ...
        function initializeMessageIds() {
            ...
            console.log('📨 Initialized message IDs:', { oldestMessageId, newestMessageId, total: messageElements.length });
        }
        ...
        function updateScrollIndicators() {
            ...
            const showTopIndicator = scrollTop > 100 && hasMoreMessages && !isLoadingMessages;
            ...
            const showBottomIndicator = scrollBottom > 100 && hasNewerMessages && !isLoadingMessages;
            if (scrollBottomIndicator) {
                scrollBottomIndicator.style.display = showBottomIndicator ? 'flex' : 'none';
            }
        ...
        let wheelTimeout;
        if (messagesArea) {
            messagesArea.addEventListener('wheel', function (e) {
                ...
                console.log('🎡 Wheel event:', { scrollTop, delta, hasMoreMessages, hasNewerMessages });

                // Throttle wheel events per migliori performance
                wheelTimeout = setTimeout(() => {
                    // Auto-load when near boundaries
                    if (scrollTop <= 150 && delta < 0 && hasMoreMessages && !isLoadingMessages) {
                        console.log('🔄 Triggering load older messages from wheel');
                        loadOlderMessages();
                    }

                    if (scrollTop + clientHeight >= scrollHeight - 150 && delta > 0 && hasNewerMessages && !isLoadingMessages) {
        ...
        if (messagesArea) {
            messagesArea.addEventListener('scroll', function () {
                ...
                scrollTimeout = setTimeout(() => {
                    ...
                    if (scrollTop + clientHeight >= scrollHeight - 100 && hasNewerMessages && !isLoadingMessages) {
        ...
        function loadOlderMessages() {
            if (isLoadingMessages || !hasMoreMessages || !oldestMessageId) {
                console.log('❌ Cannot load older messages:', { isLoadingMessages, hasMoreMessages, oldestMessageId });
                return;
            }
            ...
            fetch(url, {
                ...
                .then(data => {
                    ...
                    if (data.messages && data.messages.length > 0) {
                        ...
                        <div class="spoon-message-bubble ${msg.is_mine ? 'spoon-message-sent' : 'spoon-message-received'} spoon-message-loaded">
                    ...
                    `;
                        ...
                        console.log('✅ Updated oldestMessageId:', oldestMessageId, 'hasMoreMessages:', hasMoreMessages);
                        ...
                        setTimeout(() => {
                            messagesArea.querySelectorAll('.spoon-message-loaded').forEach(msg => {
        ...
        function loadNewerMessages() {
            if (isLoadingMessages || !newestMessageId) {
                console.log('❌ Cannot load newer messages:', { isLoadingMessages, newestMessageId });
                return;
            }
            ...
            fetch(url, {
                ...
                .then(data => {
                    ...
                    `;
                        ...
                        console.log('✅ Updated newestMessageId:', newestMessageId, 'hasNewerMessages:', hasNewerMessages);
                    } else {
                        hasNewerMessages = false;
        ...
        document.addEventListener('keydown', function (e) {
            if (e.target === messageInput || (emojiPicker && emojiPicker.classList.contains('show'))) return; // Don't interfere with typing or emoji selection
        ...
        console.log('✅ Chat initialized successfully with enhanced scrolling and persistent input bar');
    });
